...
def affiche_serpent(serpent):
    for boutSerpent in serpent: 
        x, y = case_vers_pixel(boutSerpent)

        cercle(x, y, taille_case/2 + 1,
               couleur='darkgreen', remplissage='green')

def affiche_obstacle(obstacles):
    for obstacle in obstacles:
        x, y = case_vers_pixel(obstacle)
        rectangle(x-taille_case/2, y-taille_case/2, x+taille_case/2, y+1.5*taille_case,
                  couleur='peru', remplissage='peru')
    
        
def affiche_texte():
    affichage=f'Score : {score}'
    x, y = case_vers_pixel((3,3))
    texte(x, y, affichage, couleur='black', 
          ancrage='nw', police='Arial', taille=24, tag='')
...
if __name__ == "__main__":
    ...
    serpent = [(30, 30)] # liste des coordonnées de cases adjacentes décrivant le serpent
    ...
    while jouer:
        ...
        directionStock.append(direction)
    
        #On change la position de la tête
        for i in range(len(serpent)):
            serpent[i] = (serpent[i][0] + directionStock[direc-i][0], serpent[i][1] + directionStock[direc-i][1])
        direc+=1
    
        #On vérifie les sorties de map
        if serpent[0][0] <0 or serpent[0][0] >= largeur_plateau:
            print("Partie finiez partie perdue axe x")
            jouer = False
        elif serpent[0][1] <0 or serpent[0][1] >= hauteur_plateau:    
            print("Partie finie partie perdue axe y")
            jouer = False
        elif serpent[0] in obstacles:
            print("Partie finie obstacle")
            jouer = False
    
        #Génération des pommes tous les X rafraichissements
        X=70
        if generation%X==0:
            pomme = (randint(1, largeur_plateau), randint(1, hauteur_plateau))
            #tant que la pomme est sur le serpent, on en génère une autre
            while (pomme in serpent)or (pomme in obstacles):
                pomme = (randint(1, largeur_plateau), randint(1, hauteur_plateau))
            pommes.append(pomme)
        
        #On vérifie les pommes
        if serpent[0] in pommes:
            #10 de score par pomme
            score += 10
            pommes.remove(serpent[0])
            print("On a mangé une pomme")
            serpent.append((serpent[len(serpent)-1][0] - directionStock[direc-len(serpent)][0], 
                            serpent[len(serpent)-1][1] - directionStock[direc-len(serpent)][1]))
        
        affiche_texte()
        
        #Génération d'obstacle tous les X rafraichissements
        X=100
        if generation%X==0:
            obstacle = (randint(1, largeur_plateau), randint(1, hauteur_plateau))
            #tant que la pomme est sur le serpent, on en génère une autre
            while (obstacle in serpent) or (obstacle in pommes):
                obstacle = (randint(1, largeur_plateau), randint(1, hauteur_plateau))
